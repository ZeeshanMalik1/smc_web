...
function SideNav() {
  ...
  const navLinks = [
    ...
    { title: "Convocation", path: "/admin/convocation", icon: <GraduationCap size={20} /> },
    { title: "Site Settings", path: "/admin/settings", icon: <Settings size={20} /> },
    { title: "Profile", path: "/admin/profile", icon: <User size={20} /> },
  ];

  const navLinkClass = ({ isActive }) =>
    `relative flex items-center gap-[12px] py-[12px] px-[16px] rounded-xl text-white/90 no-underline transition-all duration-300 hover:bg-white/10 hover:translate-x-2 group ${isActive
